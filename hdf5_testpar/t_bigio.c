    ...
    int mode)
{
    ...
    count[0]  =  space_dim1;
    count[1]  =  space_dim2;
    start[0]  =  (hsize_t)loc_mpi_rank*count[0];
    start[1]  =  0;
    ...
    start[0]  =  space_dim1*(hsize_t)loc_mpi_rank;
    start[1]  =  0;
    ...
    case BYROW_SELECTUNBALANCE:
      /* The first one-third of the number of processes only
        ...
        block[0]  = 1;
    count[0]  = 2;
        stride[0] = space_dim1*(hsize_t)loc_mpi_size/(hsize_t) (4+1);
        block[1]  = space_dim2;
        count[1]  = 1;
        start[1]  = 0;
        stride[1] = 1;
    if((loc_mpi_rank *3)<(loc_mpi_size*2)) start[0]  = (hsize_t)loc_mpi_rank;
    else start[0] = 1 + space_dim1*(hsize_t)loc_mpi_size/2 + ((hsize_t)loc_mpi_rank-2*(hsize_t)loc_mpi_size/3);
        break;
    ...
    case BYROW_SELECTINCHUNK:
      /* Each process will only select one chunk */
        ...
        block[0] = 1;
        count[0] = 1;
    start[0] = (hsize_t)loc_mpi_rank*space_dim1;
        stride[0]= 1;
    block[1] = space_dim2;
    count[1] = 1;
    stride[1]= 1;
    ...
    block[0]  = space_dim1*(hsize_t)loc_mpi_size;
    block[1]  = space_dim2;
    stride[0] = block[0];
    stride[1] = block[1];
    count[0]  = 1;
...
int main(int argc, char **argv)
{
    ...
    ExpressMode = EXPRESS_MODE; /* do_express_test(mpi_rank); */
