...
test_create_zero_dim_dset(void)
{
    ...
    if ((dset_id = H5Dcreate2(group_id, ZERO_DIM_DSET_TEST_DSET_NAME, H5T_NATIVE_INT, fspace_id,
            H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
...
test_create_dataset_creation_properties(void)
{
    ...
    BEGIN_MULTIPART {
        ...
        PART_BEGIN(DCPL_filters_test) {
             ...
             */
            if ((dset_id = H5Dcreate2(group_id, DATASET_CREATION_PROPERTIES_TEST_FILTERS_DSET_NAME, H5T_NATIVE_INT, fspace_id,
        ...
        PART_BEGIN(DCPL_storage_layout_test) {
            ...
            for (i = 0; i < ARRAY_LENGTH(layouts); i++) {
                ...
                if ((dset_id = H5Dcreate2(group_id, name,
...
test_dataset_builtin_type_conversion(void)
{
    ...
    BEGIN_MULTIPART {
        PART_BEGIN(H5Dwrite_all_read) {
            ...
            if (NULL == (data = HDmalloc((hsize_t) space_npoints * DATASET_DATA_BUILTIN_CONVERSION_TEST_MEM_DTYPESIZE))) {
                H5_FAILED();
                HDprintf("    couldn't allocate buffer for dataset read\n");
                PART_ERROR(H5Dwrite_all_read);
            }
        ...
        PART_BEGIN(H5Dwrite_hyperslab_read) {
            ...
            if (NULL == (read_buf = HDmalloc((hsize_t) space_npoints * DATASET_DATA_BUILTIN_CONVERSION_TEST_MEM_DTYPESIZE))) {
                H5_FAILED();
                HDprintf("    couldn't allocate buffer for dataset read\n");
                PART_ERROR(H5Dwrite_hyperslab_read);
            }
...
test_dataset_set_extent_data(void)
{
    ...
    if ((dset_id = H5Dcreate2(group_id, DATASET_SET_EXTENT_DATA_TEST_DSET_NAME, H5T_NATIVE_INT,
            fspace_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
        H5_FAILED();
...
test_dataset_set_extent_invalid_params(void)
{
    ...
    if ((compact_dset_id = H5Dcreate2(group_id, DATASET_SET_EXTENT_INVALID_LAYOUT_TEST_COMPACT_DSET_NAME, H5T_NATIVE_INT,
    ...
    if ((contiguous_dset_id = H5Dcreate2(group_id, DATASET_SET_EXTENT_INVALID_LAYOUT_TEST_CONTIGUOUS_DSET_NAME, dset_dtype,
